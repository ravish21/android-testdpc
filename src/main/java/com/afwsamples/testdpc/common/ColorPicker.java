...
/**
 * This fragment can be used to get a color input from the user. It contains the seek bars for RGB
 * and an edit box for entering hexadecimal color value, either of them can be used to provide
 * input.
 */
public class ColorPicker extends DialogFragment
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    final View rootView = LayoutInflater.from(getActivity()).inflate(R.layout.color_picker, null);
    AlertDialog dialog =
        new AlertDialog.Builder(getActivity())
            .setView(rootView)
            .setPositiveButton(
                R.string.color_picker_done,
  ...
  private void updateViewsColor() {
    ...
    mColorValue.getBackground().mutate().setColorFilter(mCurrentColor, PorterDuff.Mode.SRC_ATOP);
  ...
  public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
    if (fromUser) {
      mCurrentColor =
          Color.rgb(mRedBar.getProgress(), mGreenBar.getProgress(), mBlueBar.getProgress());
      updateViewsColor();
    }
  }
  ...
  /**
   * User has tapped the update button, update the color value depending on the value in the edit
   * box. The current color value will be updated only if the edit box contains a valid color,
   * otherwise an error toast is shown to the user.
   */
  @Override
